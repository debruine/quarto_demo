...
However, this solution doesn't work with plots that use the `theme()` function, as the dark and light themes will overwrite any custom theme (i.e., the title below should be red, but isn't in either version).

```{r toggle-with-theme, results='asis'}
